...
def parse_asset_path(asset_path: str) -> str:
    ...
    if idx != -1:
        relative = asset_path[idx + len(marker) :]
    else:
        relative = asset_path.split("/")[-1]
...
if __name__ == "__main__":
    ...
    draw_marker(bigmap, x=map_x, y=map_y, color="red", radius=20)

    # 载具兄弟-阶段2
    map_x, map_y = world_pos_to_map_pos(-101411.055, 75400.87)
    draw_marker(bigmap, x=map_x, y=map_y, color="green", radius=20)

    # 载具兄弟-阶段3
    map_x, map_y = world_pos_to_map_pos(-127812.64, 68254.4)
    draw_marker(bigmap, x=map_x, y=map_y, color="blue", radius=20)

    # 载具兄弟-阶段4/5
    map_x, map_y = world_pos_to_map_pos(-98927.58, 69289.6)
    draw_marker(bigmap, x=map_x, y=map_y, color="orange", radius=20)
